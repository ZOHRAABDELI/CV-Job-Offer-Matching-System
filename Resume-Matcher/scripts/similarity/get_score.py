...
def get_score(resume_string, job_description_string, model_name="BAAI/bge-base-en"):
    """
    The function `get_score` uses QdrantClient to calculate the similarity score between a resume and a
    job description using the specified embedding model.

    Args:
      resume_string: The string containing the text of a resume.
      job_description_string: The string containing the text of the job description.
      model_name: The name of the embedding model to use. Default is "BAAI/bge-base-en".
    ...
    Returns:
      The search result containing similarity scores between the resume and job description.
    """
    logger.info(f"Started getting similarity score using model: {model_name}")
    ...
    if model_name not in valid_models:
        logger.warning(f"Invalid model name: {model_name}. Using default model: BAAI/bge-base-en")
